...
def evaluate(args):
    ...
    model = BertAbs.from_pretrained(args.model_name)
    ...
    for batch in tqdm(data_iterator):
        ...
        # save_summaries(summaries, args.summaries_output_path, batch.document_names)

        if args.compute_rouge:
            reference_summaries += batch.tgt_str
            generated_summaries += summaries

    with open( args.summaries_output_path, "w") as output:
            output.writelines(generated_summaries)
...
def build_data_iterator(args, tokenizer):
    ...
    iterator = DataLoader(dataset, sampler=sampler, batch_size=args.batch_size, collate_fn=collate_fn, drop_last=True)

    return iterator


def load_and_cache_examples(args, tokenizer):
    dataset = CNNDataset(args.documents_path)
    return dataset
